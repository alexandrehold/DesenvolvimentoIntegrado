...
// Implementação compatível com Windows do getline()
...
double* load_csv(const char* filename, int rows, int cols)
{
    ...
    {
        ssize_t read = my_getline(&line, &len, f);
        if(read <= 0){
            printf("Erro ao ler linha %d de %s\n", i, filename);
            free(line);
            fclose(f);
            return NULL;
        }
        ...
        {
            if(token == NULL){
                printf("Erro: linha %d tem menos colunas que %d!\n", i, cols);
                free(line);
                fclose(f);
                return NULL;
            }

            data[i*cols + j] = atof(token);
            token = strtok(NULL, ",; \t\n");
        }
    }
